...
static int module_open(struct inode *inode, struct file *filp) {
  ...
  if (unlikely(!filp->private_data)) // unlikely to occur then GCC would optimzie our code like if it's unlikely to occur
    return -ENOMEM;

  INIT_LIST_HEAD((struct list_head*)filp->private_data); // set a new blob_list(two way) and set filp->private_data as the head
  return 0;
}
